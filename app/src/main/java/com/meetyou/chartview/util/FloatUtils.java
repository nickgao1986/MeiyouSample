...
public class FloatUtils {
     ...
     * @param multiple 最小倍数
     */
    public static void computeAutoGeneratedAxisValuesWithCustomStep(float start, float stop, int steps, int multiple, AxisAutoValues outValues, ChartComputator computator) {
        int range = (int) (stop - start);
        int complement = range % steps == 0 ? 0 : multiple - range % steps;
        int interval = (range + complement) / steps; // 补全使绘制range为大于初始range的最小multiple倍数

        outValues.complementHeight = range + complement;
        computator.setComplemenHeight(range + complement);

        /*Viewport currentViewport = computator.getCurrentViewport();
        computator.setMaxViewport(currentViewport.left, range + complement, currentViewport.right, currentViewport.bottom);
        computator.setCurrentViewport(currentViewport.left, range + complement, currentViewport.right, currentViewport.bottom);*/

        int valuesNum = steps + 1;
        ...
        for (int i = 0; i < valuesNum; i++) {
            outValues.values[i] = start + interval * i;
        }
    }
}
