...
public abstract class AbstractChartView extends View implements Chart {
    protected static final int DEFAULT_MARGIN_TOP_FOR_DRAWING_LABEL = 30;
    protected static final int DEFAULT_MARGIN_LEFT_FOR_DRAWING_LABEL = 2;
    protected static final int DEFAULT_MARGIN_RIGHT_FOR_DRAWING_LABEL = 2;
    ...
    // 这里的rect是供图表部分绘制,不包含坐标轴
    protected void clipRectForChart(Canvas canvas) {
        canvas.clipRect(new Rect((int) (chartComputator.getContentRectMinusAllMargins().left - density * DEFAULT_MARGIN_LEFT_FOR_DRAWING_LABEL),
                (int) (chartComputator.getContentRectMinusAllMargins().top - density * DEFAULT_MARGIN_TOP_FOR_DRAWING_LABEL),
                (int) (chartComputator.getContentRectMinusAllMargins().right + density * DEFAULT_MARGIN_RIGHT_FOR_DRAWING_LABEL),
    ...
    public float getZoomLevel() {
        ...
        return Math.max(maxViewport.width() / currentViewport.width(), maxViewport.height() / currentViewport
                .height());
    ...
    /**
     * 是否允许ChartView 选中 项，并保持状态。
     *
     * @param isValueSelectionEnabled
     */
    @Override
    ...
    /**
     * 在ChartView首次渲染的时候要延迟调用,否则不生效，View.postDelay();
     * <p>
     * 功能： 选中 selectValue项；
     *
     * @param selectedValue
    ...
    public void scale(float scale) {
        getTouchHandler().getChartZoomer().scale(
                chartComputator,
                chartComputator.getContentRectMinusAllMargins().left,
                chartComputator.getContentRectMinusAllMargins().height() / 2.0f,
                scale);
        invalidate();
    }

    @Deprecated
    public void scale(float scale, float viewportOffsetX) {
        getTouchHandler().getChartZoomer().scale(
                chartComputator,
                chartComputator.getContentRectMinusAllMargins().left,
                chartComputator.getContentRectMinusAllMargins().height() / 2.0f,
                scale, viewportOffsetX);
        invalidate();
    }
    ...
    public void setAxisDegreeMargin(int margin) {
        getAxesRenderer().setAxisDegreeMargin(margin);
    }
     ...
     * @param margin
     */
    public void setAxisHorizontalLeftMargin(float margin) {
        getAxesRenderer().setAxisHorizontalLeftMargin(margin);
    }
     ...
     * @param hide
     */
    public void setHideFirstAxisLabel(boolean hide) {
        getAxesRenderer().setHideFirstAxisLabel(hide);
    }

    /**
     * 设置 高亮坐标 "今天"
     *
     * @param highLightAxisLabel
     */
    public void setHighLightAxisLabel(boolean highLightAxisLabel) {
        getAxesRenderer().setHighLightAxisLabel(highLightAxisLabel);
    }
