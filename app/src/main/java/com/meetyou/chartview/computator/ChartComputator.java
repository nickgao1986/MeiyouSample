...
import com.meetyou.chartview.listener.DummyVieportChangeListener;
...
public class ChartComputator {
    ...
    public float computeAxisRawY(float valueY, int complementHeight) {
        float pixelOffset = 0;
        try {
            if (complementHeight > 0) {
                pixelOffset = (valueY - currentViewport.bottom) * (contentRectMinusAllMargins.height() / complementHeight);
            } else {
                pixelOffset = (valueY - currentViewport.bottom) * (contentRectMinusAllMargins.height() / currentViewport.height());
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public int getComplemenHeight() {
        return complemenHeight;
    }

    public void setComplemenHeight(int complemenHeight) {
        this.complemenHeight = complemenHeight;
    }
}