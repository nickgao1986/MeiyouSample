 ...
 */
public class InterestFriendAdapter extends BaseAdapter {
    public List<AddFriendModel> friendModels;
    public Activity mContext;
    private LayoutInflater mInflater;


    public InterestFriendAdapter(Activity mContext, List<AddFriendModel> models) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = mInflater.inflate(R.layout.layout_follow_recommend_friend_item,
                    null);
        ...
        if (position == 0) {
            RelativeLayout.LayoutParams params = (RelativeLayout.LayoutParams) holder.rlBase
                    .getLayoutParams();
            // params.topMargin = DeviceUtils.getMarginTopWithoutWave(mContext.getApplicationContext());
...
//        SkinEngine.getInstance().setViewBackground(mContext,holder.rlBase,R.drawable.apk_all_white_selector);
        ...
        /*
        if (position == 0) {
            SkinEngine.getInstance().setViewBackground(mContext,holder.rlBase,R.drawable.apk_all_spread_kuang_top_selector);
        } else if (position == getCount() - 1) {
            SkinEngine.getInstance().setViewBackground(mContext,holder.rlBase,R.drawable.apk_all_spread_kuang_bottom_selector);
        } else {
            SkinEngine.getInstance().setViewBackground(mContext,holder.rlBase,R.drawable.apk_all_white_selector);
        }
        if (getCount() == 1) {
            SkinEngine.getInstance().setViewBackground(mContext,holder.rlBase,R.drawable.apk_all_spread_kuang_selector);
        }
        */
