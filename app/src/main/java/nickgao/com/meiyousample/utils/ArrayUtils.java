...
public class ArrayUtils {
     ...
     *
     * @param list
     * @param des  0 时间小的排前面 1 时间大大排前面
     * @return
     */
    public static List<Calendar> sortList(List<Calendar> list, int des) {
        // LogUtils.i(TAG,"排序前结果为："+list.toString());
        if (des == 0) {
            Collections.sort(list, new Comparator() {

                @Override
                public int compare(Object lhs, Object rhs) {
                    return ((Calendar) lhs).getTime().compareTo(
                            ((Calendar) rhs).getTime());
                }
            });
        } else if (des == 1) {
            Collections.sort(list, new Comparator() {
                ...
                @Override
                public int compare(Object lhs, Object rhs) {
                    return ((Calendar) rhs).getTime().compareTo(
                            ((Calendar) lhs).getTime());
                }
            });
        }

        // LogUtils.i(TAG,"排序后结果为："+list.toString());
        return list;
    }
