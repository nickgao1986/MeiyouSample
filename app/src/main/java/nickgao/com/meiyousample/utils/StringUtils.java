...
public class StringUtils {
     ...
     * 批量查询，是否包含
     *
     * @param str       eg： sdfsdfsdf
     * @param searchStr sdf,wesd;
     * @return
     */
    public static boolean contains(String str, String... searchStr) {
        if (str == null || searchStr == null) {
...
*/
    ...
    /**
     * get
     * 过滤掉无意义的0，比如0012，00.12，12.00之类的0
     *
     * @param editText
     * @return
     */
    public static String filterUnMeanZero(String editText) {
        ...
        if (pointIndex > 0) {
            ...
            if (StringUtils.isNotBlank(s2) && s2.length() > 1) {
                s2 = editText.substring(pointIndex + 1);
                if (Double.parseDouble(s2) == 0.00) {
                    s2 = "";
                } else {
                    if (s2.length() > 1) {
     ...
     * @return
     */
    public static JSONObject mapToJson(TreeMap<String, String> map) {
    ...
    public static JSONArray getJsonArray(List<String> fileName)
            throws Exception {
        ...
        for (int i = 0; i < fileName.size(); i++) {
            ...
            if (!TextUtils.isEmpty(file)) {
                String name = getFileNameByFullPathName(file);
                array.put(name);
            }
        }
        return array;
     ...
     * @return
     */
    public static boolean hasNumAndWord(String str) {
        boolean bl = false;
        if (isEmpty(str)) {
            return bl;
        }
    ...
    public final static String convertMd5(String src) {
        ...
        try {
            ...
            //LogUtils.d("mdt 16bit: " + buf.toString().substring(8, 24));
            LogUtils.d("md5 32bit: " + buf.toString());
        } catch (Exception e) {
            e.printStackTrace();
        }
        return result;
    ...
    public static String getFileNameByFullPathName(String strFilePathName) {
        try {
            String strFileName = "";
            if (TextUtils.isEmpty(strFilePathName))
                return "";
            String[] array = strFilePathName.split("/");
            if (array != null) {
                strFileName = array[array.length - 1];
                return strFileName;
            }
    ...
    public static String getPriceFormat(String price) {
        ...
        try {
            ...
            String distanceString = decimalFormat.format(value);
    ...
    public static String getSessionId(String msg_from, String msg_to) {
        int from = 0, to = 0;
    ...
    public static boolean getJsonBooleanDefaultTure(JSONObject object, String key) {
    ...
    public static int getJsonSkinInt(JSONObject object, String key) {
        int strValue = -1;
        try {
            if (object == null || key == null)
                return strValue;
     ...
     * @param size
     */
    public static int getStringPaintWidth(String string, float size) {
        if (isEmpty(string)) {
            return 0;
        }
        Rect rect = new Rect();
     ...
     * @param url
     */
    public static boolean isTaobaoItemDetailUrl(String url) {
        if (!TextUtils.isEmpty(url) && url.contains(".taobao.com")) {
            int index = url.indexOf('?');
            if (index > 0 && ((url.substring(0, index).contains("/detail."))
                    || url.substring(0, index).contains("http://detail."))) {
                return true;
            }
        }
        return false;
     ...
     * @param url
     */
    public static boolean isTmallItemDetailUrl(String url) {
        if (!TextUtils.isEmpty(url) && url.contains(".tmall.com")) {
            int index = url.indexOf('?');
            if (index > 0 && ((url.substring(0, index).contains("/detail."))
                    || url.substring(0, index).contains("http://detail."))) {
                return true;
     ...
     * @param url
     */
    public static boolean isTmallOrTaobaoUrl(String url) {
        if (!TextUtils.isEmpty(url) && (url.contains(".tmall.com") || url.contains(".taobao.com"))) {
            return true;
        }
        return false;
    }
     ...
     * @param style
     */
    public static void setTextOneSize(Context context, String msg, TextView tvContent, int style) {
        SpannableStringBuilder spannableStringBuilder = new SpannableStringBuilder(msg);
        spannableStringBuilder.setSpan(new TextAppearanceSpan(context, style), 0, 1, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        tvContent.setText(spannableStringBuilder);//将其添加到tv中
    }
     ...
     * @return
     */
    public static JSONObject getParamsJson(TreeMap<String, String> map) {
        JSONObject obj = new JSONObject();
        for (Map.Entry<String, String> entry : map.entrySet()) {
            try {
                obj.put(entry.getKey(), entry.getValue());
