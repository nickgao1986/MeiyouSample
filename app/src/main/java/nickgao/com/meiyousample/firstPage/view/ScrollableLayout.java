...
public class ScrollableLayout extends RelativeLayout {
    ...
    private boolean isOnTouchUp, isAutoScroll;
    ...
    private ValueAnimator stopScrollAnim;
    //刷新回调
    ...
    private void init(Context context) {
        ...
        homeStyle = HomeType.HOME_STYLE_ONE;
     ...
     * @param ivBannerBg
     */
    public void setLoaddingView(RelativeLayout rl_update, RelativeLayout rl_loadding, ImageView ivBannerBg) {
        clearUIStatus();
        this.mScaleView = ivBannerBg;
        this.rl_loadding = rl_loadding;
        this.rl_update = rl_update;

        this.ballLoaddingView = (TheBallLoaddingView) rl_update.findViewById(R.id.ballLoaddingView);
        this.tvLoadding = (TextView) rl_update.findViewById(R.id.tvLoadding);

        tvLoaddingFinish = (TextView) rl_loadding.findViewById(R.id.tvLoaddingFinish);
        ivLoaddingBg = (ImageView) rl_loadding.findViewById(R.id.ivLoaddingBg);
    }
...
//                                mScroller.computeScrollOffset();
     ...
     * 设置viewpager是否可以翻页滚动
     */
    private void setViewPagerIsFlip() {
        //设置viewpager是否可以滑动
        if (isSticked()) {
     ...
     * 停止滚动的时候要滚动到卡住状态
     */
    private void stopScrollSticked() {
        if (!isSticked() && mCurY >= headViewHeight / 2) {//滑动到一半的时候
            if (isOnTouchUp) {
                isOnTouchUp = false;
                isAutoScroll = true;
                mScroller.abortAnimation();
                mScroller.startScroll(mScroller.getFinalX(), mCurY, mScroller.getFinalX(), headViewHeight - mCurY, 500);
                postInvalidate();
                scrollbleDownUp = false;
                childViewPager.setScrollble(true);
            }
    ...
    private void downAnimation(int topHeight) {
        ...
        //刷新改变高度
        LayoutParams layoutParams = (LayoutParams) rl_update.getLayoutParams();
    ...
    private void moveUpAnimation() {
        ...
        moveUp.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                int value = (Integer) animation.getAnimatedValue();
                LayoutParams layoutParams = (LayoutParams) rl_loadding.getLayoutParams();
    ...
    public void computeScroll() {
        if (mScroller.computeScrollOffset()) {
            ...
            if (mDirection == DIRECTION.UP) {
                // 手势向上划
                if (isSticked() && !isAutoScroll) {//导航栏贴边了
    ...
    public void scrollTo(int x, int y) {
        ...
        if (onScrollListener != null) {
            onScrollListener.onScroll(y, headViewHeight);
        }
        super.scrollTo(x, y);
    }
