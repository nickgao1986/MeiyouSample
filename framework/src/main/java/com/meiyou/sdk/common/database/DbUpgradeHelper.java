...
public class DbUpgradeHelper {
     ...
     * @return
     */
    private static String buildString(Object... str) {
        StringBuilder builder = new StringBuilder();
        for (Object aStr : str) {
            builder.append(String.valueOf(aStr));
        }
    ...
    public static void checkTableAllColumn(DbManager dbManager, Class<?> entityType) {
        try {
            ...
            HashMap<String, Column> needAdd = new HashMap<>();
            if (classFieldMap != null) {
                //获取model需要创建的所有字段
                for (Map.Entry<String, Column> entry:classFieldMap.entrySet()) {
                    int index= cursor.getColumnIndex(entry.getKey());
                    if(index == -1){ //不在表里
                        needAdd.put(entry.getKey(),entry.getValue());
                    }
                }
                if(needAdd.size()>0){
                    DbUpgradeHelper.addNewCoulumns(dbManager, entityType, needAdd);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
